...
#include "hybrid_node.h" // TEMPORARILLY until Tobi finishes his thesis
...
//                            postorder
//          postorder_vector  A reference to a vector which is filled with the
//                            traversed nodes in postorder
template<class _NodeData = nodes::StringNodeData>
void postorder(nodes::Node<_NodeData>* root,
  std::vector<nodes::Node<_NodeData>*>& postorder_vector)
{
  if (root) {
    // traverse children first
    if (root->get_children_number() > 0) {
      for (int i = 0; i < root->get_children_number(); ++i) {
        postorder(root->get_child(i), postorder_vector);
      }
    }

    postorder_vector.push_back(root);
  }
}
...
//           postorder_vector A reference to a vector of node pointers which is
//                            filled with the nodes in postorder
template<class _NodeData = nodes::StringNodeData>
void generate_postorder(nodes::Node<_NodeData>* root,
  std::vector<nodes::Node<_NodeData>*>& postorder_vector)
{
  ...
  postorder(root, postorder_vector);
}
